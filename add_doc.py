...
class class_add_doc:
    def __init__(self,root):
        ...
        #=== TITLE ===
        title = Label(self.root,text="DOCTOR REGISTRATION SECTION",font=("times new roman",40),bg="white",fg="#ff8b33",anchor="w",padx=20).place(x=300,y=70,relwidth=0.7,height=70)

        #=== ADDING LABEL FRAMES ===
        LabelFrame(self.root,text="Doctor's Details",font=5,bg='white').place(x=130,y=170,relwidth=0.6,relheight=0.6)
        #LabelFrame(self.root,text="KK Hospital",font=5,bg="white").place(x=1100,y=170,relheight=0.6,relwidth=0.23)
        ...
        lbl_doc_id = Label(self.root,text="Doctor ID",font=('calibri',20),bg='white').place(x=160,y=230)
        en_doc_id = Entry(self.root,textvariable=self.var_doc_id,width=25,border=0,font=('Arial',16)).place(x=295,y=232)
        ...
        lbl_spec = Label(self.root,text='Speciality',font=('calibri',20),bg='white').place(x=600,y=430)
        en_specs = ttk.Combobox(self.root,textvariable=self.var_specs,values=("-- Select --","Cardiology","Neurology","Urology","Dermatology","Orthopaedics","Pediatrics"),font=20,state='readonly',justify=CENTER,width=20)
        en_specs.place(x=735,y=430)
        en_specs.current(0)

        lbl_addr = Label(self.root,text="Address",font=('calibri',20),bg='white').place(x=160,y=510)
        en_addr = Entry(self.root,textvariable=self.var_addr,width=20,border=0,font=('Arial',16)).place(x=295,y=510)
        Frame(root,width=250,height=2,bg="#141414").place(x=295,y=537)
